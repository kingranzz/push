...
module.exports = async (
  ...
  const hText = `⚡\x20*${global.name}\x20ヅ*\x20|\x20*BROADCAST*\n\n`;
  const fText = `\n*⌱\x20${getDateTime()}*\n`;
  ...
  if (target.length <= 0) {
    setTimeout(() => {
      pk.sendMessage(
        id,
        {
          text: `${hText}*BROADCAST GAGAL*\x20‼️\n*Filter Kontak Status: _${fcStatus}_*\n*Filter History Status: _${fhStatus}_*\n*Target:*\x20${
            target.length
          }\n*Jeda:*\x20${jedaBc / 1000}s\n*Elapsed:*\x20${
            (jedaBc * target.length) / 1000
          }s\n*Limit:*\x20${limitBc}\n*Pesan:*\x20Target ${
            target.length
          }\n*Text:*\x20${textBc}\n${fText}`,
  ...
  const loop = setInterval(async () => {
    if (index >= target.length) {
      logger(
        "success",
        "BROADCAST",
        `Broadcast sukses, ${index} pesan telah berhasil dikirim.`
      );
      await pk.sendMessage(msg.key.remoteJid, {
        text: `${hText}*BROADCAST SUKSES*\x20✅\n*Filter Kontak Status: _${fcStatus}_*\n*Filter History Status: _${fhStatus}_*\n*Participants:*\x20${
          participants.length
        }\n*Target:*\x20${target.length}\n*Jeda:*\x20${
          jedaBc / 1000
        }s\n*Elapsed:*\x20${
      ...
      try {
        if (msgType === "imageMessage") {
          ...
          await pk.sendMessage(target[index], { text: textBc });
        }
